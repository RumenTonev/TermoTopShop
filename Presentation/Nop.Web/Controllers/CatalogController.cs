...
//I have meet the same issue on Nopcommerce 2.65.

//and I have tried add some code on "Category" action of "CatalogController":


//{
                int mainPictureSize = _mediaSettings.MaximumImageSize;
               // var categoryPictureCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_PICTURE_MODEL_KEY, x.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
                var mainCategoryPictureCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_PICTURE_MODEL_KEY, model.Id, mainPictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);

                model.PictureModel = _cacheManager.Get(mainCategoryPictureCacheKey, () =>
                           {
                               Category c = _categoryService.GetCategoryById(model.Id);
                               var pictureModel = new PictureModel()
                               {
                                   FullSizeImageUrl = _pictureService.GetPictureUrl(c.PictureId),
                                   ImageUrl = _pictureService.GetPictureUrl(c.PictureId, mainPictureSize),
                                   Title = string.Format(_localizationService.GetResource("Media.Category.ImageLinkTitleFormat"), c.Name),
                                   AlternateText = string.Format(_localizationService.GetResource("Media.Category.ImageAlternateTextFormat"), c.Name)
                               };
                               return pictureModel;
                           });
            //}
